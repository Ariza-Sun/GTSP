...
DATA_PATHS = {
    ...
    "ParkingCorr":{"feat": "data/SH_Park_10.csv", "adj": "data/888_SH_adj.csv"},
    "ParkingAIDD":{"feat": "data/SH_Park_10.csv", "adj": "data/aidd_adj.csv"}
}
...
def main_supervised(args):
    ...
    model = get_model(args, dm) # init a model
    task = get_task(args, model, dm)  # init a task
    callbacks = get_callbacks(args) # record checkpoints
    trainer = pl.Trainer.from_argparse_args(args, callbacks=callbacks) # use pl to train model
    trainer.fit(task, dm)
...
def main(args):
    ...
    results = globals()["main_" + args.settings](args)  # run main_supervised
    return results
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data", type=str, help="The name of the dataset", choices=("shenzhen", "losloop","ParkingCorr","ParkingAIDD"), default="ParkingCorr"
    )
    parser.add_argument(
        "--model_name",
        type=str,
